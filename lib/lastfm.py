...
def initCache():
    ...
    rcache.install_cache(
        cache_name="request-cache/lastfm", backend="sqlite", expire_after=300
    )
    global session
...
def getPages(config):
    ...
    response = getReq(
        page=1, user=user, key=key, useragent=useragent, load={"limit": 200}
    )
    total_pages = int((response.json())["recenttracks"]["@attr"]["totalPages"])
